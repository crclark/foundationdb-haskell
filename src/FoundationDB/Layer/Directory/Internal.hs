...
open dl path layer _prefix = do
  ...
  if nodeExists existingNode
    then do
      ...
      Just <$> contentsOfNodeSubspace dl
    ...
    Nothing -> do
      prefixToUse <- case prefix of
        Nothing -> do
          ...
          liftIO $ traceIO $ "passed isPrefixEmpty for " ++ show path
          when (not isPrefixEmpty)
               (throwDirError "Failed to alloc new dir: prefix not empty.")
          isFree <- isPrefixFree dl (pack newDirPrefix [])
          when (not isFree)
               (throwDirError $ "A manually allocated prefix conflicts with the one chosen by the allocator: "
                                ++ (show newDirPrefix))
      ...
      parentNode <- if length path > 1
                       then do pd <- createOrOpen dl (init path) "" Nothing
                               let pdb = pack (dirSubspace pd) []
                               return $ nodeWithPrefix dl pdb
                       else return rootNode
      --TODO: null check of parentNode here in Go code
      let node = nodeWithPrefix dl prefixToUse
      liftIO $ traceIO $ "setting " ++ show (last path)
      set (pack parentNode [ Tuple.IntElem _SUBDIRS
                           , Tuple.BytesElem (last path)])
          prefixToUse
...
-- does not exist.
list :: DirectoryLayer -> [ByteString] -> Transaction (Maybe [ByteString])
list dl path = do
  node <- find dl path
  if nodeExists node
  ...
  if oldPath == take sliceEnd newPath
    then return (Just SelfSubDir)
      ...
      case (nodeExists oldNode, nodeExists newNode, nodeExists parentNode) of
        ...
        _ -> do
          let k = pack (fromJust $ nodeNodeSS parentNode)
                       [ Tuple.IntElem _SUBDIRS
                       , Tuple.BytesElem (last newPath)]
          let ve = unpack (nodeSS dl)
                          (rawPrefix $ fromJust $ nodeNodeSS oldNode)
          case ve of
...
removeRecursive dl@DirectoryLayer{..} node = do
  ...
  case p of
    ...
    Right (Tuple.BytesElem p':_) -> case rangeKeys <$> prefixRange p' of
      ...
      Nothing -> throwDirError $ "removeRecursive: couldn't make prefix range:"
                                 ++ show p'
...
removeFromParent dl path = do
  ...
  case parent of
    ...
    (Node (Just sub) _ _) ->
      clear (pack sub [Tuple.IntElem _SUBDIRS, Tuple.BytesElem (last path)])
...
subdirNameNodes :: DirectoryLayer
                -> Subspace
                -- ^ node
                -> Transaction [(ByteString, Subspace)]
subdirNameNodes dl@DirectoryLayer{..} node = do
  let sd = node <> subspace [Tuple.IntElem _SUBDIRS]
  rr <- getRange (subspaceRange sd) >>= await
  kvs <- go rr
  return (map (\(k,v) -> (k, nodeWithPrefix dl v)) kvs)
...
subdirNames :: DirectoryLayer -> Subspace -> Transaction [ByteString]
...
nodeContainingKey dl@DirectoryLayer{..} k
  ...
  | otherwise = do
    let r = Range (rangeBegin $ subspaceRange nodeSS)
                  (FirstGreaterOrEq
                    (pack nodeSS [Tuple.BytesElem k] <> "\x00"))
                  (Just 1)
                  True
    rr <- getEntireRange r
    case rr of
    ...
    case nck of
      ...
      Nothing -> case prefixRange prefix of
        ...
        Just r -> do
          ...
          let r' = Range (FirstGreaterOrEq (pack nodeSS [Tuple.BytesElem bk]))
...
nodeWithPrefix DirectoryLayer{..} prefix =
  nodeSS <> subspace [Tuple.BytesElem prefix]
...
find :: DirectoryLayer
     ...
     -- ^ path
     -> Transaction Node
find dl@DirectoryLayer{..} queryPath =
  go baseNode (zip [0..] queryPath)

  where
    baseNode = Node (Just rootNode) [] queryPath
    go n [] = return n
    go n ((i, p):ps) = do
      let nodePrefixKey = pack (fromJust $ nodeNodeSS n) [ Tuple.IntElem _SUBDIRS
                                                         , Tuple.BytesElem p]
      nodePrefix <- get nodePrefixKey >>= await
      let n' = Node (fmap (nodeWithPrefix dl) nodePrefix) (take (i+1) queryPath) []
      if not (nodeExists n')
         then return n'
         else do
...
contentsOfNodePartition :: DirectoryLayer
  -> Subspace
  -- ^ node
  -> [ByteString]
  -- ^ path
...
contentsOfNodePartition dl@DirectoryLayer{..} node queryPath = do
  ...
  let newDL = newDirectoryLayer (subspace [Tuple.BytesElem (prefix <> "\xfe")])
                                contentSS
                                False
...
contentsOfNodeSubspace dl@DirectoryLayer{..} node queryPath layer = do
  ...
  --TODO: not total
  let newPath = dlPath <> queryPath
  return $ DirSubspace (subspace (take 1 p)) dl newPath layer
