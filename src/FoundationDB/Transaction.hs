...
awaitInterruptible fut@(Future _f e) =
  futureIsReady fut >>= \case
    True -> e
...
awaitInterruptibleIO fut =
  futureIsReadyIO fut >>= \case
    True -> awaitIO fut
...
get key = do
  ...
  withTransactionPtr $ \tp ->
    allocFuture
      (FDB.transactionGet tp key isSnapshot)
...
withRetry db t = catchError t $ \err -> do
  ...
  if shouldRetry err && retriesRemaining > 0
    then do
      ...
      local
        (\e -> e {envConf = (envConf e) {maxRetries = retriesRemaining - 1}})
        (withRetry db t)
    else do
...
getReadVersion =
  withTransactionPtr $ \t ->
    allocFuture
      (FDB.transactionGetReadVersion t)
...
createTransactionEnv DB.Database {databasePtr} config =
  ExceptT $
...
onError (CError err) =
  withTransactionPtr $ \trans -> do
    f <-
      allocFuture
        (FDB.transactionOnError trans (toCFDBError err))
        (const $ return ())
    await f
onError _ = return ()
